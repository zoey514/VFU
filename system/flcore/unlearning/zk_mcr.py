"""ZK-ready proof specification for sparse MCR correctness.

This module defines the proof relation and a deterministic prototype verifier
for MCR correctness. It is not a production zkSNARK implementation unless a
real backend is plugged in. The relation covers only masked contribution
removal arithmetic; it does not prove local training correctness, full
federated aggregation correctness, or retained-client repair correctness.
"""
...
from flcore.unlearning.auditfu import TensorDict, json_digest, subtract_state_dict, tensor_digest
...
class ZKMCRPrototypeVerifier:
    ...
    def verify(
        ...
        """Check that theta_after equals theta_before - strength * masked_contribution."""
        ...
        max_abs_error = 0.0
        for name in expected_delta.keys():
            error = (observed_delta[name].float() - expected_delta[name].float()).abs().max().item()
            max_abs_error = max(max_abs_error, float(error))
        output_matches = _state_commitment(theta_after) == proof.output_commitment
        verified = bool(max_abs_error <= atol and output_matches)
        return {
            "verified": verified,
            "mode": proof.mode,
